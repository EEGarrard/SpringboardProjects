...
console.log(`Significant locations: the ${first} and the ${second}`);
...
const { upper, middle = locations.first, lower } = doorCode;
...
const [fire, water] = elements;
console.log(`Essential elements: ${fire}, ${water}`);
...
console.log(`Extracted stones: ${one}, ${six}`);
...
console.log(`Visible shadow: ${visible}, Hidden shadows: ${hidden.join(", ")}`);
...
function revealPath({ direction, distance }) {
  console.log(`The path goes ${direction} for ${distance} km.`);
}

revealPath({ direction: "East", distance: 156 });
...
const owlList = ["springWater", "fairyDust", "spiderLegs", "rosePetals"];
function castSpell() {
  const [ingredient1, ingredient2] = owlList;
  console.log(`Casting 'Dazzle' with ${ingredient1} and ${ingredient2}`);
}
castSpell();
